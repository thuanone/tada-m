 ...
 */
export function convertValueToNumber(val: string) {
  let result;
...
export function convertValueToBytes(val: string): number {

  const result = convertValueToNumber(val);
 ...
 */
export function convertNumberToDisplayValueAndUnit(value: any, useSiUnits: boolean = true, suffix?: string): string {
...
export function convertBytesToUnit(value: number, unit: string): number {
  if (!unit) {
    return -1;
  }
 ...
 * @param targetUnit
 */
export function convertBytesToDisplayValue(bytes: number, targetUnit: string): number {
